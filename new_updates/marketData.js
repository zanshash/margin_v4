...
(function (global) {
  ...
  function getGreeks(legs) {
    ...
    legs.forEach(function (l) {
      var inst = INSTRUMENTS[l.symbol]; if (!inst) return;
      var qty = l.lots * inst.lotSize; var sign = (l.tradeType === "SELL" ? -1 : 1);
      if (l.product === "Futures") { g.delta += sign * qty; return; }
      var spot = inst.spot, strike = l.strike, days = Math.max(expiryByDate(l.expiry).days, 1);
      var t = days / 365;
      var denom = spot * 0.06 * Math.sqrt(days / 30) || 1;
    ...
    return { delta: Math.round(g.delta * 100) / 100, theta: Math.round(g.theta), gamma: Math.round(g.gamma * 10000) / 10000, vega: Math.round(g.vega) };
  }
